...
test('Perlin noise values', () => {
  ...
  expect(perlin0.valueOf()).toBeCloseTo(0.239)
})

test('Perlin noise values', () => {
  const perlin1 = new PerlinNoise(5.5, 3.7)

  expect(perlin1.valueOf()).toBeCloseTo(-0.158)
})

test('Perlin noise values', () => {
  const perlin2 = new PerlinNoise(10.0, 20.0)

  expect(perlin2.valueOf()).toBeCloseTo(0.107)
})

test('Perlin noise values', () => {
  const perlin3 = new PerlinNoise(100.0, 200.0)

  expect(perlin3.valueOf()).toBeCloseTo(0.294)
})

test('Perlin noise values', () => {
  const perlin4 = new PerlinNoise(0.5, 0.5)
  expect(perlin4.valueOf()).toBeCloseTo(0.467)
})
